...
const GOV_ADDRESS = "0xEC568fffba86c094cf06b22134B23074DFE2252c";
...
const getProposalStateString = (proposalStateId) => {
  return PROPOSAL_STATES[proposalStateId];
};
...
exports.handler = async function (credentials) {
  ...
  console.log("Timestamp:", await getCurrentTimestamp());

  const gov = new ethers.Contract(GOV_ADDRESS, GOV_ABI, signer);

  const proposalsCount = await gov.getProposalsCount();
  console.log("ProposalsCount:", proposalsCount.toString());

  let alreadyExecuted = 0;
  let id = proposalsCount.toNumber() - 1;
  do {
    const proposalStateId = await gov.getProposalState(id);
    console.log(
      `Processing proposal ${id.toString()} (${getProposalStateString(
        proposalStateId
      )})...`
    );

    if (proposalStateId == 4) {
      // Succeeded [4]: check if queueable
      const tx = await gov.queue(id);
      await tx.wait();
      console.log(`- Proposal queued (txhash: ${tx.hash})`);
    } else if (proposalStateId == 5) {
      // Queued [5]: check if executable
      const proposal = await gov.getProposalById(id);
      const executionTime = proposal.executionTime.toNumber();
      console.log(
        `- Execution time: ${executionTime} (${getHumanReadableTimestamp(
          executionTime
        )})`
      );
      const currentTime = await getCurrentTimestamp();
      console.log(`- CurrentTime: ${currentTime}`);
      const executable = executionTime <= currentTime;
      console.log(executable ? "- Executable" : "- Non Executable");
      if (executable) {
        const tx = await gov.execute(id);
        await tx.wait();
        console.log(`- Proposal executed (txhash: ${tx.hash})`);
      }
    } else if (proposalStateId > 3) {
      // Else [>3]: after 5 proposals, finish.
      alreadyExecuted++;
    }
    id--;
  } while (alreadyExecuted < 5);
